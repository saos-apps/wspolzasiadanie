...
judges.top.c<-function(data.judges){
  temp<-count(data.judges,"JudgeName")
...
j.year<-function(subset.judges.c){
  temp<-count(subset.judges.c,c("JudgeName","judgmentYear"))
  names(temp)[3]<-"dd"
...
plot.sex.distribution<-function(subset.j.clean){
  sexc<-count(subset.j.clean,c("judgmentID","JudgeSex"))
  ...
  ctemp<-count(subset.j.clean,c("JudgeName","JudgeSex"))
